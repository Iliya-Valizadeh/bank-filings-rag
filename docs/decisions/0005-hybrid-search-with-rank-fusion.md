...
commit `5ec3c84`, listed hybrid search as a next step. Its reason: financial questions
use exact terms such as "CET1" and "ROE", and keyword matching helps with those.

Commit `1f4db7f` added keyword search (BM25) and a hybrid of the two. The two searches
...
- Dense search only. What the first version did.
...
- A weighted sum of the two scores. It needs the scales matched first, and the weights
  need tuning.
- Reciprocal rank fusion. Each piece scores 1/(60 + rank) for each list it appears in,
...
hybrid search the setup the command line tool uses, because it had the best hit@5 on
all 30 questions.
...
- Hybrid search runs both searches, so it needs the embedding model as well as the
  keyword index.
